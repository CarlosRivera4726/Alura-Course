...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		btnsiguiente.addMouseListener(new MouseAdapter() {
			@Override
		    public void mouseEntered(MouseEvent e) {
		        // Actualizar el valor del TextField con el valor calculado
		        verificar("ver");
		    }
	...
	//Código que permite mover la ventana por la pantalla según la posición de "x" y "y"	
	 private void headerMousePressed(java.awt.event.MouseEvent evt) {
	    ...
	    private void verificar(String estado) {
	    	Date fechaEntrada = txtFechaEntrada.getDate();
	        Date fechaSalida = txtFechaSalida.getDate();
	    	if(estado.equalsIgnoreCase("guardar") && ( fechaEntrada != null && fechaSalida != null)) {
		    				        

			        Date fechaActualSinHora = new Date();
			        Calendar calendar = Calendar.getInstance();
			        calendar.setTime(fechaActualSinHora);
			        calendar.set(Calendar.HOUR_OF_DAY, 0);
			        calendar.set(Calendar.MINUTE, 0);
			        calendar.set(Calendar.SECOND, 0);
			        calendar.set(Calendar.MILLISECOND, 0);
			        fechaActualSinHora = calendar.getTime();
	
			        if (fechaEntrada.compareTo(fechaActualSinHora) >= 0) {
			        	...
			        	Reserva reserva = new Reserva(FECHA_ENTRADA, FECHA_SALIDA , VALOR, FORMA_PAGO);
			            // la fecha de hoy o superior
			        	ReservaController reservaController = new ReservaController();
			            reserva = reservaController.guardar(reserva);
			            JOptionPane.showMessageDialog(null, "La reserva se ha guardado correctamente con id: " + reserva.getId());
			            RegistroHuesped registro = new RegistroHuesped();
			            registro.pasarReserva(reserva);
			            registro.setVisible(true);
			        }
			        else {
			            JOptionPane.showMessageDialog(null, "Debes seleccionar una fecha superior o igual a la del dia de hoy.");
			            return;
			        }
	
	    	} 
	    	else if(estado.equalsIgnoreCase("ver") && ( fechaEntrada != null && fechaSalida != null)) 
	    	{
